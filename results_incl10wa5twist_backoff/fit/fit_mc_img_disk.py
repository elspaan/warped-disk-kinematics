...
#SOME DEFINITIONS
#*********************
foldername = 'Linas_cubes/fits-and-analysis/incl10_02-twisted-wa5/'
diskname = 'disk_name'
file_data = f'../image_lines_incl10_02-twisted-wa5_mod_noisy_convolved_clipped_downsamp_8pix.fits'
tag_out = 'incl10wa5twist_12co_backoff_mod' #PREFERRED FORMAT: disc_mol_chan_program_extratags
tag_in = tag_out
...
# Orientation
incl = np.deg2rad(-10) # positive = tilted towards us along the redshifted axis
PA = np.deg2rad(0) # Joe: offset by about 270 deg; positive = anti-clockwise; wrong sign gives a 90 degree rotation clockwise for some reason. Also divide by 2pi if PA > 2pi
xc = 0.0
yc = 0.0

# intensity
I0 = 0.003 # norm. cte, Jy/pix
p = -1.1 # R-power
q = 0.7 # z-power
# Rbreak = 100. # radius for powerlaw break, default is 100 au
Rout = 280 # extend of the disk, au

# line width
L0 = 0.3 # norm. cte, km/s
pL = -1.0 # R-power
qL = 0.4 # z-power
...
z0_upper, p_upper, Rb_upper, q_upper = 30, 1.6, 250, 5.0 # norm. cte, R-power1 (flare), taper point radius, R-power2 (exp. taper); au
z0_lower, p_lower, Rb_lower, q_lower = 20, 1.6, 250, 5.0 # same definition as upper surface
...
model.mc_params['height_lower'] = {'z0': 0, 'p': 0, 'Rb': 1, 'q': 0}
...
if __name__ == '__main__': # this fixes a weird bug. Indent is important.
    ...
    model.run_mcmc(datacube.data, vchannels,
           ...
           backend=backend,
           tag=tag_out,
           nthreads=112, # If not specified considers maximum possible number of cores
           frac_stats=0.1, # analogous to the burnin time, chooses which part to take for the corner plot
           frac_stddev=1e-2, # estimates stdev 
           noise_stddev=noise) 
