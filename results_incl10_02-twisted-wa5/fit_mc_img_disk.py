...
tag_out = 'synthcube_12co_incl10_02-twisted-wa5_mod' #PREFERRED FORMAT: disc_mol_chan_program_extratags
tag_in = tag_out
...
#****************************
model = Model(datacube, Rmax, Rmin=0, prototype =True) # set inner Rmin to 0, can also be 1 (then it starts from 1 beam size)
...
# Orientation
incl = -0.175 # positive = tilted towards us along the redshifted axis
...
if __name__ == '__main__': # this fixes a weird bug. Indent is important.
    ...
    model.run_mcmc(datacube.data, vchannels,
           ...
           backend=backend,
           tag=tag_out,
           nthreads=132, # If not specified considers maximum possible number of cores
